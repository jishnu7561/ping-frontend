...
//             {}
//         ).then((response)=>{
//             console.log("chatting:" ,response.data)
...
function ChatPageContent() {
    ...
    useEffect(()=>{
        request("GET",
            ...
            console.log("all users on chatting: ",response.data)
            setUserDetails(response.data)
            setLoading(false)
        })
    },[showArrowIcon]);
    ...
    const seacrhHandler = async (e) => {
        ...
        try {
          ...
          if (response.data) {
            setUserDetails(response.data)
            console.log("userDetails response in chat: ",response.data);
          } else {
            console.log(response)
          }
        } catch (error) {
    ...
    var stompClient = null;
// let socket = new SockJS('http://localhost:8085/ws'); // Replace with your server endpoint
let socket = new SockJS('https://chat.cravehub.site/ws'); 
stompClient = over(socket);
...
function onConnected() {
  console.log('Connected to WebSocket');
  stompClient.subscribe(`/chatting-users`, updatedMessagReceived);
}

useEffect(() => {

    // request("PUT",`/chat/isRead/chatId=${userDetails.chatId}`)
  stompClient.connect({}, onConnected, onError);
...
const updatedMessagReceived =(payload)=> {
  console.log("called...");
  const payloadData = JSON.parse(payload.body);
  console.log("new chattings : ",payloadData)
  setUserDetails(prevUserDetails => {
    ...
    const updatedUserDetails = prevUserDetails.map(user => {
        ...
        if (user.chatId === payloadData.chatId) {
            return {
                ...user,
                lastMessage: payloadData.content,
                unreadMessage: (user.unreadMessage || 0) + 1
                // lastMessageDate: payloadData.lastMessageDate
            };
        }
        return user; // Return unchanged if no match
                    ...
                    {userDetails.map((data, index) => (
                        ...
                        <div key={index} className='border-b-2 border-light_gray px-3 py-3 cursor-pointer' onClick={() => handleUserClick(data)}>
                            <div className='flex gap-3 items-center'>
                                ...
                                <div className='bg-green w-4 h-4 rounded-3xl flex justify-center items-center'>
                                    <p className='text-white text-xs'>{data.unreadMessage}</p>
                                </div>
                                }
                            </div>
                            {/* <div className='bg-grey w-full h-96' >
